...
import { getSupabase, supabaseConfigured, type ReferenceLetterRow } from "@/lib/supabase";
import { referenceLetters } from "@/data/site";
import { SectionTitle } from "./SectionTitle";

const staticItems: ReferenceLetterRow[] = referenceLetters.map((m, i) => ({
  id: `static-${i}`,
  url: m.src,
  alt: m.alt ?? null,
  media_type: m.type,
...
export function ReferenceLetters() {
  const [items, setItems] = useState<ReferenceLetterRow[]>(staticItems);
  const [open, setOpen] = useState<number | null>(null);

  useEffect(() => {
    if (!supabaseConfigured) return;
    const sb = getSupabase();
    sb.from("reference_letters")
      .select("*")
      .eq("visible", true)
      .order("sort_order", { ascending: true })
      .then(({ data }) => {
        const rows = (data ?? []) as ReferenceLetterRow[];
  ...
  return (
    <section id="cartas-referenciales" className="py-20 md:py-28 bg-background">
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        <SectionTitle
          eyebrow="Cartas Referenciales"
          title="Voces que respaldan nuestra obra"
          subtitle="Cartas y testimonios de quienes han caminado con nosotros y han querido dejar su palabra."
        />

        {items.length === 0 ? (
          <p className="mt-12 text-center text-muted-foreground">
            Pronto compartiremos las primeras cartas.
          </p>
        ) : (
                ...
                {item.media_type === "image" ? (
                  ...
                  <>
                    <video
                      ...
                      className="h-full w-full object-contain group-hover:scale-105 transition-transform duration-500 bg-slate-100"
                    />
          ...
          >
            ...
            {current.media_type === "image" ? (
              <motion.img
                ...
                exit={{ opacity: 0 }}
                src={current.url}
                alt={current.alt ?? "Carta ampliada"}
